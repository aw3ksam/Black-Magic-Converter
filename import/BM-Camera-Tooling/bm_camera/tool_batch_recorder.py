...
Configures camera resolution, codec/bitrate, and executes the automated recording sequence.
"""
...
logger = logging.getLogger("bm_camera.tool2")
...
class BatchRecorderTool:
    ...
    def _emit(self, event_type: str, data: Dict[str, Any]):
        ...
        if self.on_event_cb:
            try:
                ...
                logger.error(f"Error in batch tool event callback: {e}")
        ...
        try:
            ...
            for fmt in formats:
                ...
                resolution_options.append({
                    "width": w,
                    "height": h,
                    "label": f"{group} - {description} ({w}x{h}, {aspect})",
                    "codecs": codecs,
    ...
    def start_batch(
        ...
        with self._lock:
            ...
            self.target_clips = (
                custom_clip_count if custom_clip_count and custom_clip_count > 0
            ...
            self._thread = threading.Thread(target=self._batch_worker_loop, daemon=True, name="Tool2-BatchWorker")
            self._thread.start()
    ...
    def resume_batch(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            self._pause_event.clear()
            self.state = "cooldown"  # will resume on next cycle
