...
logger = logging.getLogger("bm_camera.tool1")
...
class AutoTransferTool:
    ...
    def activate(self, auto_import_same_day: bool = False) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            self._monitor_thread = threading.Thread(target=self._monitor_loop, daemon=True, name="Tool1-Monitor")
            self._worker_thread = threading.Thread(target=self._worker_loop, daemon=True, name="Tool1-Worker")
    ...
    def import_same_day_clips(self) -> Dict[str, Any]:
        ...
        and queue any missing ones for download.
        """
        today_str = datetime.now().strftime("%m%d")  # e.g., '0902' or '0807'
        ...
        try:
            ...
            for c in clips:
                ...
                # Also check if destination file already exists
                local_file = self.dest_dir / fname
                already_downloaded = local_file.exists() and local_file.stat().st_size >= file_size > 0

                if (is_today or auto_check_all_today(c, today_iso)) and not already_downloaded:
            ...
            for c in clips:
                ...
                if cid not in self.known_clip_ids and fname not in self.known_file_names:
                    if cid not in self._queued_clip_ids:
                        ...
                        self._transfer_queue.put(clip_item)
                        self._queued_clip_ids.add(cid)
                        # Add to known clips so we don't re-queue it again
                        self.known_clip_ids.add(cid)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dest_dir": str(self.dest_dir.resolve()),
...
def auto_check_all_today(clip_obj: Dict[str, Any], today_iso: str) -> bool:
    """Check if clip object date matches today."""
    date_str = clip_obj.get("date", "")
    if today_iso in date_str:
        return True
    return False
