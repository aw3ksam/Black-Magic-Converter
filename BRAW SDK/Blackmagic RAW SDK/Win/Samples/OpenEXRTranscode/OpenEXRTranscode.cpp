...
static bool ExrTimecodeFromString(const std::string& timecodeString, Imf::TimeCode& timecode)
{
	...
	try
	{
		...
		if (sscanf_s(timecodeString.c_str(), "%02d:%02d:%02d:%02d", &hours, &mins, &secs, &frames) != 4)
			return false;
...
static void BmdStringToStdString(const BSTR& bmdString, std::string& stdString)
{
	std::wstring wstr(bmdString, SysStringLen(bmdString));

	// find out how many bytes will be used in the UTF-8 string by passing a destination buffer size of 0
	int numBytesToCopy = WideCharToMultiByte(CP_UTF8, 0, &wstr[0], (int)wstr.size(), 0, 0, 0, 0);

	// Copy in the new string
	stdString.resize(numBytesToCopy, 0);
	WideCharToMultiByte(CP_UTF8, 0, &wstr[0], (int)wstr.size(), &stdString[0], numBytesToCopy, 0, 0); // actually copy into the destination string
}
...
static void VariantToStdString(const VARIANT& value, std::string& stdString)
{
	...
	if (value.vt != blackmagicRawVariantTypeString)
		BmdStringToStdString(value.bstrVal, stdString);
}
...
static HRESULT GetFrameMetadata(IBlackmagicRawFrame* frame, ExrMetadata& metadata)
{
	...
	do
	{
		...
		if (FAILED(result))
			break;
		BmdStringToStdString(timecode, metadata.timecode);
		SysFreeString(timecode);

		// Shutter Value
		BSTR shutterValue = SysAllocString(L"shutter_value");

		VariantClear(&value);
		result = frame->GetMetadata(shutterValue, &value);
		SysFreeString(shutterValue);
		if (FAILED(result))
			break;
		...
		// Focal Length
		BSTR focalLength = SysAllocString(L"focal_length");

		VariantClear(&value);
		result = frame->GetMetadata(focalLength, &value);
		SysFreeString(focalLength);
		if (FAILED(result))
			break;
		...
		// Aperture
		BSTR aperture = SysAllocString(L"aperture");

		VariantClear(&value);
		result = frame->GetMetadata(aperture, &value);
		SysFreeString(aperture);
		if (FAILED(result))
			break;
...
static HRESULT DecodeFrame(IBlackmagicRawClip* clip, const std::string& folder, const std::string& prefix, uint64_t frameIndex, const ExrMetadata& metadata)
{
	...
	sprintf_s(buff, sizeof(buff), "_%05llu.exr", frameIndex);
...
static bool ProcessCommandLine(int argc, const char* argv[], bool& showCompressionOptions)
{
	...
	brawFile = std::filesystem::absolute(brawFile, ec).string();
	...
	if (std::filesystem::is_directory(brawFile))
	{
		...
		for (const auto& entry : std::filesystem::directory_iterator(s_brawFolder))
		{
			...
			if (path.extension() == ".braw")
				s_brawFiles.push_back(path.filename().string());
		}
	}
	else
	{
		...
		s_brawFolder = path.parent_path().string();
		if (! endsWith(s_brawFolder, '/'))
			s_brawFolder.append("/");
	...
	else
	{
		...
		s_exrFolder = std::filesystem::absolute(exrFolder, ec).string();
...
static void ShowFeedbackProgress(const std::string& brawFilename, uint64_t frameCount)
{
	...
	int length = printf("Transcoding File: %4u of %4llu %s  %5.1f%% Frame %5llu of %5llu\r", s_filesTranscodedCount+1, s_brawFiles.size(), brawFilename.c_str(), percent, s_framesDecodedCount.load(), frameCount);
...
static HRESULT SetDecodeOptions(IBlackmagicRawClip* clip)
{
	...
	do
	{
		...
		value.bstrVal = SysAllocString(L"Linear");
		...
		VariantInit(&value);
		value.vt = blackmagicRawVariantTypeString;
		value.bstrVal = SysAllocString(L"ACES AP0");
		...
		VariantInit(&value);
		value.vt = blackmagicRawVariantTypeString;
		value.bstrVal = SysAllocString(L"Disabled");
...
static HRESULT ProcessBrawFile(IBlackmagicRaw* codec, const std::string brawFilename)
{
	...
	do
	{
		...
		if (! s_singleFrameOnly)
		{
			folder.append(path.stem().string());
			folder.append("/");
		}
		...
		int strLength = (int)clipFilename.length();
		int wslen = MultiByteToWideChar(CP_ACP, 0, clipFilename.c_str(), (int)clipFilename.length(), 0, 0);
		BSTR clipName = SysAllocStringLen(0, wslen);

		MultiByteToWideChar(CP_ACP, 0, clipFilename.c_str(), strLength, clipName, wslen);
