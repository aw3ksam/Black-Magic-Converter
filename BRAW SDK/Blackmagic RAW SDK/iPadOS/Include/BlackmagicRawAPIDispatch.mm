...
IBlackmagicRawFactory* CreateBlackmagicRawFactoryInstanceFromBundle (void)
{
	...
	if (gCreateBlackmagicRawFactoryInstance == NULL)
	{
		...
		{
			NSURL* frameworksURL = mainBundle.privateFrameworksURL;
			if (frameworksURL != nullptr)
			{
				NSURL* apiFrameworkURL = [frameworksURL URLByAppendingPathComponent:@"BlackmagicRawAPI.framework"];

				TryLoadBlackmagicRawAPI((__bridge CFURLRef)apiFrameworkURL);
			}
			else
			{
				NSLog(@"frameworksURL == Null");
			}
		}
		else
			NSLog(@"mainBundle == Null");
	}
	if (gCreateBlackmagicRawFactoryInstance == NULL)
	{
		NSLog(@"apiBundleRef == Null");
		return NULL;
	}
