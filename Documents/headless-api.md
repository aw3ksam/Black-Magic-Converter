...
![DaVinci_Resolve_Studio.png](DaVinci_Resolve_Studio.png)

# DaVinci Resolve Scripting API - Documentation

_Formatted and Maintained by [X-Raym](https://www.extremraym.com) - [Source](https://gist.github.com/X-Raym/2f2bf453fc481b9cca624d7ca0e19de8) - [Release Article](https://www.extremraym.com/resolve-api-doc-release) - [Forum Thread](https://forum.blackmagicdesign.com/viewtopic.php?f=12&t=210618) - [X-Raym Free Resolve Scripts](https://github.com/X-Raym/DaVinci-Resolve-Scripts) - [Donation](https://www.paypal.me/extremraym/)_ ❤️

**Important:** Without any mention in the v19.1 changelog, **Blackmagic just removed UIManager for the free version of Resolve**, breaking all scripts using UI for free users, including the well-known [Reactor](https://www.steakunderwater.com/wesuckless/viewtopic.php?t=3067). **[Please join the discussion about this](https://forum.blackmagicdesign.com/viewtopic.php?f=12&t=213158)** on [BM Forum](https://forum.blackmagicdesign.com/viewtopic.php?f=12&t=213158) to help bring it back!
...
Starts a quick export render for the current active timeline. preset\_name from GetQuickExportRenderPresets list. param\_dict supports render settings keys `TargetDir`, `CustomName`, `VideoQuality`, and `EnableUpload`.  
...
Generates an audio MediaPoolItem based on the given speechGenerationSettings and adds it to the timeline at the stated timecode if `AddToTimeline` is True. Returns the newly generated MediaPoolItem.  
...
Adds list of itemInfos specified as dict of `media`, `startFrame` (int), `endFrame` (int) from Media Storage into current Media Pool folder. Returns a list of the MediaPoolItems created.
...
Example: ImportMedia(\[{`FilePath`:"file\_%03d.dpx", `StartIndex`:1, `EndIndex`:100}\]) would import clip "file\_\[001-100\].dpx".
...
Applies motion deblur on MediaPoolItems in the folder. Returns a list of pairs mapping original to newly created MediaPoolItems.  
...
Returns a string with MediaPoolItem's audio mapping information. Check 'Audio Mapping' section below for more information.
...
Adds track of trackType (`video`, `subtitle`, `audio`). Optional newTrackOptions = {`audioType`: same as subTrackType above, `index`: 1 <= index <= GetTrackCount(trackType))  
...
An example of how to retrieve and interpret thumbnails is provided in 6\_get\_current\_media\_thumbnail.py in the Examples folder.
...
set analysisType to resolve.DLB\_BLEND\_SHOTS for blend setting  
Refer to section "Studio and AI Scripting APIs" for prerequisites.
...
Sets Voice Isolation state of audio track with given trackIndex (int) to the given VoiceIsolationState of {isEnabled (bool), amount (int)}. amount is in range of \[0, 100\] and 1 <= trackIndex <= GetTrackCount(`audio`). Returns True if successful.  
Refer to section "Studio and AI Scripting APIs" for prerequisites.
...
Returns the item duration. Returns fractional frames if subframe\_precision is True

`GetEnd(subframe_precision)`

int/float

Returns the end frame position on the timeline. Returns fractional frames if subframe\_precision is True
...
Returns the maximum extension by frame for clip from left side. Returns fractional frames if subframe\_precision is True

`GetRightOffset(subframe_precision)`

int/float

Returns the maximum extension by frame for clip from right side. Returns fractional frames if subframe\_precision is True

`GetStart(subframe_precision)`

int/float

Returns the start frame position on the timeline. Returns fractional frames if subframe\_precision is True
...
Copies the current node stack layer grade to the same layer for each item in tgtTimelineItems. Returns True if successful.
...
Returns True if TiItem to successfully assigned to given ColorGroup. ColorGroup must be an existing group in the current project.
...
Returns if the cache corresponding to cache\_type is enabled

`GetIsFusionOutputCacheEnabled()`

cache\_value

Returns if the cache corresponding to cache\_type is enabled (or auto)
...
Sets Voice Isolation state of the timelineItem to the given VoiceIsolationState of {isEnabled (bool), amount (int)}. amount is in range of \[0, 100\]. Returns True if successful.  
Refer to section "Studio and AI Scripting APIs" for prerequisites.
...
Exports list of GalleryStill objects '\[galleryStill\]' to directory `folderPath`, with filename prefix `filePrefix`, using file format `format` (supported formats: dpx, cin, tif, jpg, png, ppm, bmp, xpm, drx).
...
Sets the cache mode type on the node mapping the node index provided. Refer to section "Cache Mode" below to find the possible values of cache\_value.
...
Returns a list of TimelineItem that are in colorGroup in the given Timeline. Timeline is Current Timeline by default.
...
This section covers additional notes for the functions Graph:GetNodeCacheMode(nodeIndex) and Graph:SetNodeCacheMode(nodeIndex, cache\_value).

cache\_value is an enumerated integer with one of the following values:
...
With AUDIO\_SYNC\_WAVEFORM mode, channelNumber is used to determine channel offset for comparison. channelNumber can be one of the following:
...
-   `Pan` : floating point values from -4.0_width to 4.0_width
-   `Tilt` : floating point values from -4.0_height to 4.0_height
...
-   `AnchorPointX` : floating point values from -4.0_width to 4.0_width
-   `AnchorPointY` : floating point values from -4.0_height to 4.0_height
...
-   `DynamicZoomEase` : A value from the following constants
    -   DYNAMIC\_ZOOM\_EASE\_LINEAR = 0
    -   DYNAMIC\_ZOOM\_EASE\_IN
    -   DYNAMIC\_ZOOM\_EASE\_OUT
    -   DYNAMIC\_ZOOM\_EASE\_IN\_AND\_OUT
-   `CompositeMode` : A value from the following constants
    ...
    -   COMPOSITE\_SCREEN
    -   COMPOSITE\_OVERLAY
    -   COMPOSITE\_HARDLIGHT
    -   COMPOSITE\_SOFTLIGHT
    -   COMPOSITE\_DARKEN
    -   COMPOSITE\_LIGHTEN
    -   COMPOSITE\_COLOR\_DODGE
    -   COMPOSITE\_COLOR\_BURN
    -   COMPOSITE\_EXCLUSION
    -   COMPOSITE\_HUE
    -   COMPOSITE\_SATURATE
    -   COMPOSITE\_COLORIZE
    -   COMPOSITE\_LUMA\_MASK
    -   COMPOSITE\_DIVIDE
    -   COMPOSITE\_LINEAR\_DODGE
    -   COMPOSITE\_LINEAR\_BURN
    -   COMPOSITE\_LINEAR\_LIGHT
    -   COMPOSITE\_VIVID\_LIGHT
    -   COMPOSITE\_PIN\_LIGHT
    -   COMPOSITE\_HARD\_MIX
    -   COMPOSITE\_LIGHTER\_COLOR
    -   COMPOSITE\_DARKER\_COLOR
    -   COMPOSITE\_FOREGROUND
    -   COMPOSITE\_ALPHA
    -   COMPOSITE\_INVERTED\_ALPHA
    -   COMPOSITE\_LUM
    -   COMPOSITE\_INVERTED\_LUM
...
-   `RetimeProcess` : A value from the following constants
    -   RETIME\_USE\_PROJECT = 0
    -   RETIME\_NEAREST
    -   RETIME\_FRAME\_BLEND
    -   RETIME\_OPTICAL\_FLOW
-   `MotionEstimation` : A value from the following constants
    -   MOTION\_EST\_USE\_PROJECT = 0
    -   MOTION\_EST\_STANDARD\_FASTER
    -   MOTION\_EST\_STANDARD\_BETTER
    -   MOTION\_EST\_ENHANCED\_FASTER
    -   MOTION\_EST\_ENHANCED\_BETTER
    -   MOTION\_EST\_SPEED\_WARP\_BETTER
    -   MOTION\_EST\_SPEED\_WARP\_FASTER
-   `Scaling` : A value from the following constants
    -   SCALE\_USE\_PROJECT = 0
    -   SCALE\_CROP
    -   SCALE\_FIT
    -   SCALE\_FILL
    -   SCALE\_STRETCH
-   `ResizeFilter` : A value from the following constants
    -   RESIZE\_FILTER\_USE\_PROJECT = 0
    -   RESIZE\_FILTER\_SHARPER
    -   RESIZE\_FILTER\_SMOOTHER
    -   RESIZE\_FILTER\_BICUBIC
    -   RESIZE\_FILTER\_BILINEAR
    -   RESIZE\_FILTER\_BESSEL
    -   RESIZE\_FILTER\_BOX
    -   RESIZE\_FILTER\_CATMULL\_ROM
    -   RESIZE\_FILTER\_CUBIC
    -   RESIZE\_FILTER\_GAUSSIAN
    -   RESIZE\_FILTER\_LANCZOS
    -   RESIZE\_FILTER\_MITCHELL
    -   RESIZE\_FILTER\_NEAREST\_NEIGHBOR
    -   RESIZE\_FILTER\_QUADRATIC
    -   RESIZE\_FILTER\_SINC
    -   RESIZE\_FILTER\_LINEAR Values beyond the range will be clipped width and height are same as the UI max limits
...
This section covers the supported settings for the method AnalyzeForSlate(markerColor) markerColor can be one of the following constants:
...
`AddItemsToMediaPool(item1, item2, ...)`

{clips...}

Adds specified file/folder paths from Media Storage into current Media Pool folder. Input is one or more file/folder paths. Returns a dict of the MediaPoolItems created.

`AddItemsToMediaPool([items...])`

{clips...}

Adds specified file/folder paths from Media Storage into current Media Pool folder. Input is an array of file/folder paths. Returns a dict of the MediaPoolItems created.
...
`DeleteRenderJobByIndex(idx)`

Bool

Please use unique job ids (string) instead of indices.

`GetRenderJobStatus(idx) GetSetting and SetSetting`
