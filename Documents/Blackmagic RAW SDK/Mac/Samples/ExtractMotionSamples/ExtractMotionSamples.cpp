...
std::string ConvertCFStringRefToStdString(CFStringRef cfString)
{
	...
	const bool success = CFStringGetCString(cfString, buffer, length + 1, kCFStringEncodingUTF8);
...
std::string GenerateTimecode(IBlackmagicRawClip* clip, uint64_t motionSampleIndex, float motionSampleRate, float videoFrameRate)
{
	...
	const HRESULT result = clip->GetTimecodeForFrame(videoFrameIndex, &timecodeCFString);

	if (result == S_OK)
	{
		timecodeString = ConvertCFStringRefToStdString(timecodeCFString);
...
{
	const std::string defaultOutputFileName = "/tmp/outputMotionSamples.csv";
	...
	CFStringRef clipNameCFString = CFStringCreateWithCString(NULL, clipName.c_str(), kCFStringEncodingUTF8);
