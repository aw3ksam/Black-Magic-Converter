...
static void BmdStringToStdString(const CFStringRef& string, std::string& stdString)
{
	char* buffer = NULL;
	...
	try
	{
		long len = 1 + CFStringGetMaximumSizeForEncoding(CFStringGetLength(string), kCFStringEncodingUTF8);
		buffer = new char[len];
		if (buffer)
		{
			CFStringGetCString(string, buffer, len, kCFStringEncodingUTF8);
...
static HRESULT GetFrameMetadata(IBlackmagicRawFrame* frame, ExrMetadata& metadata)
{
	...
	do
	{
		...
		// Shutter Value
		CFStringRef shutterValue = CFStringCreateWithCString(NULL, "shutter_value", kCFStringEncodingUTF8);;

		VariantClear(&value);
		result = frame->GetMetadata(shutterValue, &value);
		CFRelease(shutterValue);
		if (FAILED(result))
			break;
		VariantToStdString(value, metadata.shutterValue);
		CFRelease(value.bstrVal);

		// Focal Length
		CFStringRef focalLength = CFStringCreateWithCString(NULL, "focal_length", kCFStringEncodingUTF8);;

		VariantClear(&value);
		result = frame->GetMetadata(focalLength, &value);
		CFRelease(focalLength);
		if (FAILED(result))
			break;
		...
		// Aperture
		CFStringRef aperture = CFStringCreateWithCString(NULL, "aperture", kCFStringEncodingUTF8);;
...
static HRESULT DecodeFrame(IBlackmagicRawClip* clip, const std::string& folder, const std::string& prefix, uint64_t frameIndex, const ExrMetadata& metadata)
{
	...
	sprintf(buff, "_%05llu.exr", frameIndex);
...
static void ShowFeedbackProgress(const std::string& brawFilename, uint64_t frameCount)
{
	...
	int length = printf("Transcoding File: %4u of %4lu %s  %5.1f%% Frame %5llu of %5llu\r", s_filesTranscodedCount+1, s_brawFiles.size(), brawFilename.c_str(), percent, s_framesDecodedCount.load(), frameCount);
...
static HRESULT SetDecodeOptions(IBlackmagicRawClip* clip)
{
	...
	do
	{
		...
		value.bstrVal = CFStringCreateWithCString(NULL, "Linear", kCFStringEncodingUTF8);
		...
		VariantInit(&value);
		value.vt = blackmagicRawVariantTypeString;
		value.bstrVal = CFStringCreateWithCString(NULL, "ACES AP0", kCFStringEncodingUTF8);
		...
		VariantInit(&value);
		value.vt = blackmagicRawVariantTypeString;
		value.bstrVal = CFStringCreateWithCString(NULL, "Disabled", kCFStringEncodingUTF8);
...
static HRESULT ProcessBrawFile(IBlackmagicRaw* codec, const std::string brawFilename)
{
	...
	do
	{
		...
		CFStringRef clipName = CFStringCreateWithCString(NULL, clipFilename.c_str(), kCFStringEncodingUTF8);
