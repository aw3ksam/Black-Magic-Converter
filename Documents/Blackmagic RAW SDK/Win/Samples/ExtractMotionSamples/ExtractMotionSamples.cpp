...
BSTR ConvertCStringToBSTR(const char* cString)
{
	uint32_t cStringLength = uint32_t(strlen(cString));
	uint32_t bstrLength = uint32_t(MultiByteToWideChar(CP_ACP, 0, cString, cStringLength, 0, 0));
	BSTR bstr = SysAllocStringLen(0, bstrLength);

	MultiByteToWideChar(CP_ACP, 0, cString, cStringLength, bstr, bstrLength);
...
{
	uint32_t bstrLength = uint32_t(SysStringLen(bstr));
	uint32_t stdStringLength = uint32_t(WideCharToMultiByte(CP_UTF8, 0, bstr, bstrLength, NULL, 0, NULL, NULL));
	std::string stdString(stdStringLength, 0);

	WideCharToMultiByte(CP_UTF8, 0, bstr, bstrLength, &stdString[0], stdStringLength, NULL, NULL);
...
std::string GenerateTimecode(IBlackmagicRawClip* clip, uint64_t motionSampleIndex, float motionSampleRate, float videoFrameRate)
{
	...
	const HRESULT result = clip->GetTimecodeForFrame(videoFrameIndex, &timecodeBSTR);

	if (result == S_OK)
	{
		timecodeString = ConvertBSTRToStdString(timecodeBSTR);
...
int main(int argc, const char* argv[])
{
	...
	BSTR clipNameBSTR = ConvertCStringToBSTR(clipName.c_str());
	...
	do
	{
		...
		result = codec->OpenClip(clipNameBSTR, &clip);
